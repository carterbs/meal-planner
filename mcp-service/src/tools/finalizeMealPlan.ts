...
export function registerFinalizeMealPlan(server: McpServer) {
    server.tool('finalizeMealPlan', 'Finalize the provided meal plan and make it the active meal plan. This commits the planned meals and makes them permanent until a new plan is generated.', {
        mealPlan: {
            description: 'The meal plan to finalize',
            type: 'object'
        }
    }, async (args) => {
        const { mealPlan } = args as {
            mealPlan: any;
        };
