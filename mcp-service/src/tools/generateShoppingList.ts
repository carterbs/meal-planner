...
export async function generateList(plan: number[]): Promise<GetShoppingListResponse> {
    try {
        ...
        if (!resp.ok) {
            ...
            throw new McpError(-32000, `BackendError: ${resp.status} ${resp.statusText} - ${errorData.message || 'Unknown error'}`);
        }
        ...
        await errorLog(`🛒 [MCP] Error generating shopping list: ${error}`);
        throw error;
    }
}
import { z } from 'zod';
...
export function registerGenerateShoppingList(server: McpServer) {
    server.tool('generateShoppingList', 'Generate a comprehensive shopping list based on the current meal plan. This analyzes all planned meals and creates a consolidated list of ingredients with quantities needed for the week.', {
