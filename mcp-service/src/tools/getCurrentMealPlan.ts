...
import { WeeklyMealPlan, GetMealPlanResponse } from '@mealplanner/generated';
...
export function registerGetCurrentMealPlan(server: McpServer) {
    server.tool('getCurrentMealPlan', 'Get the current weekly meal plan showing all scheduled meals by day and type (breakfast/lunch/dinner). Provides context for understanding what meals are currently planned and which ones might need replacement.', async () => {
