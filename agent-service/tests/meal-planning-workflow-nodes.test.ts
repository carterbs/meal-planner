...
describe('MealPlanningWorkflow Core Node Tests', () => {
  ...
  describe('generatePlanNode', () => {
    it('generates meal plan from MCP tool successfully', async () => {
      ...
      mockClient.callTool.mockResolvedValue({
        isError: false,
        content: [{ type: 'text', text: JSON.stringify({ plan: mockPlan.toJson() }) }],
      });
    ...
    it('handles MCP tool errors during plan generation', async () => {
      ...
      await expect(workflow.generatePlanNode(mockState)).rejects.toThrow('MCP tool error: MCP tool error');
    });
  });
  ...
  describe('presentPlanNode', () => {
    ...
    it('throws error when no meal plan to present', async () => {
      ...
      await expect(workflow.presentPlanNode(mockState)).rejects.toThrow('No meal plan to present');
    });
  });
  ...
  describe('finalizePlanNode', () => {
    ...
    it('handles MCP tool errors during plan finalization', async () => {
      ...
      mockClient.callTool.mockRejectedValue(new Error('MCP finalization failed'));
    ...
    it('throws error when no meal plan to finalize', async () => {
      ...
      await expect(workflow.finalizePlanNode(mockState)).rejects.toThrow('No meal plan to finalize');
    });
  });
  ...
  describe('generateShoppingListNode', () => {
    it('generates shopping list from meal plan', async () => {
      ...
      const mockShoppingList = [
        TestMockFactory.createMockShoppingListItem({ ingredient: 'Tomatoes', category: 'Vegetables' }),
        TestMockFactory.createMockShoppingListItem({ ingredient: 'Chicken', category: 'Meat' }),
      ];
    ...
    it('handles MCP tool errors during shopping list generation', async () => {
      ...
      mockClient.callTool.mockRejectedValue(new Error('Shopping list generation failed'));
    ...
    it('throws error when no meal plan for shopping list generation', async () => {
      ...
      await expect(workflow.generateShoppingListNode(mockState)).rejects.toThrow('No meal plan for shopping list generation');
    });
  });
  ...
  describe('completeNode', () => {
    ...
    it('handles completion with validation issues', async () => {
      ...
      const problemMealPlan = TestMockFactory.createMockWeeklyMealPlan(
        TestMockFactory.createHighEffortMeals(4) // Too many consecutive high-effort meals
      );
