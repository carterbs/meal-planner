import { formatMealPlanForClipboard, copyMealPlanToClipboard, copyShoppingListToClipboard } from './clipboard';
import { WeeklyMealPlan, MealPlanEntry, Meal, ShoppingListItem } from '@mealplanner/generated';
...
describe('clipboard utils', () => {
    function buildPlan(): WeeklyMealPlan {
        ...
        const entries: MealPlanEntry[] = [
            new MealPlanEntry({ dayIndex: 0, mealType: 'breakfast', meal: breakfast }),
    ...
    it('copies shopping list with rich clipboard when available', async () => {
        const items = [
            new ShoppingListItem({ ingredient: 'Milk', quantity: '2', category: 'Dairy' }),
