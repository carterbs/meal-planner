...
function makePlan(entries: Array<{ dayIndex: number; mealType: string; mealId?: number }>): WeeklyMealPlan {
    const days = entries.map(
        (e) => new MealPlanEntry({ dayIndex: e.dayIndex, mealType: e.mealType, meal: e.mealId ? new Meal({ id: e.mealId, name: `Meal ${e.mealId}`, effort: 1 }) : undefined }),
    );
...
describe('MealPlanDisplay highlights', () => {
    ...
    it('renders highlight state when provided and visually fades (non-visual assert via data attribute)', () => {
        ...
        const { rerender } = render(<MealPlanDisplay plan={plan} highlights={highlights} />);
