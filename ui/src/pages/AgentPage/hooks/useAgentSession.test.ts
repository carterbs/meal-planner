...
jest.mock('../../../api', () => ({
    __esModule: true,
    startAgentSession: jest.fn().mockResolvedValue({ session: { threadId: 't1', currentStep: '' }, message: 'hi' }),
}));

jest.mock('../../../hooks/useSession', () => ({
    __esModule: true,
    default: () => ({ isResuming: false, resumeData: undefined, startNewSession: jest.fn() }),
}));
...
describe('useAgentSession', () => {
    it('sets session from resumeData and supports logout', async () => {
        ...
        jest.doMock('../../../hooks/useSession', () => ({
            __esModule: true,
            default: () => ({ isResuming: false, resumeData: { threadId: 't1', currentStep: '' }, startNewSession }),
        }));
        const { default: useAgentSessionRemocked } = await import('./useAgentSession');
