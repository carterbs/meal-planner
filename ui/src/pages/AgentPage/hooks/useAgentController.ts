...
export default function useAgentController(): UseAgentControllerReturn {
    ...
    const { messages: fetchedMessages, fetchMessages } = useAgentMessages(session?.threadId);
    const { mealPlan, shoppingList, syncFromCheckpoint, send, setMealPlan } = useAgentMealSync();

    const startSession = useCallback(async () => {
        const result = await start();
        if (result.initialState?.mealPlan) {
            setMealPlan(result.initialState.mealPlan as any);
        }
        if (result.message) {
    ...
    return useMemo(
        ...
        [session, startSession, _logout, input, isStarting, messages, sendMessage, mealPlan, shoppingList, setMealPlan]
    );
}
