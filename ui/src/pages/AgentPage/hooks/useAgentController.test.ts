...
const mockStartAgentSession = jest.fn().mockResolvedValue({ session: { threadId: 't1', currentStep: '' }, message: 'hello' });
const mockSendAgentMessage = jest.fn().mockResolvedValue({});
const mockGetAgentCheckpoint = jest.fn().mockResolvedValue({ state: { mealPlan: { days: [] } } });
const mockGetMessages = jest.fn().mockResolvedValue([{ sender: 'agent', content: 'hi' }]);
...
jest.mock('../../../hooks/useSession', () => ({
    __esModule: true,
    default: () => ({ isResuming: false, resumeData: { threadId: 't1', currentStep: '' }, startNewSession: jest.fn() }),
}));
...
jest.mock('@mealplanner/generated', () => ({
    ...
    ShoppingListItem: class ShoppingListItem {
        ...
        quantity: string;
        category: string;
        constructor(args: { ingredient?: string; quantity?: string; category?: string }) {
