...
const ChatMessages: React.FC<ChatMessagesProps> = ({ messages, isWorking, styles }) => {
            ...
            {messages.map((message, index) => (
                <Box key={index} sx={styles.messageContainer(message.sender === 'user')}>
                    <Box sx={styles.messageContent(message.sender === 'user')}>
                        <Avatar sx={styles.avatar}>{message.sender === 'agent' ? 'AI' : 'You'}</Avatar>
                        <Box sx={styles.messageBubble(message.sender === 'user')}>
                            <Typography
                                variant="body2"
                                sx={{ lineHeight: 1.5, fontSize: '0.9375rem', wordBreak: 'break-word', whiteSpace: 'pre-wrap' }}
                            >
                                {message.text}
                            </Typography>
                        </Box>
