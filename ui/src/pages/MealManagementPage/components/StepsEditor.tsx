...
const SortableItem: React.FC<SortableItemProps> = ({
  ...
  return (
    <ListItem ref={setNodeRef} style={style} divider={index < steps.length - 1}>
      <Box
...
const StepsEditor: React.FC<StepsEditorProps> = ({
  ...
  const [stepInputMode, setStepInputMode] = useState<'bulk' | 'individual'>(
    'individual',
  );
            ...
            {steps.map((step, index) => (
              <React.Fragment key={step.id || index}>
                <ListItem alignItems="flex-start">
                  <ListItemText primary={`${index + 1}. ${step.instruction}`} />
                </ListItem>
