...
export const MealPlanTab: React.FC<MealPlanTabProps> = ({ showToast }) => {
  ...
  const swapMeal = (day: string) => {
    ...
    fetch("/api/meals/swap", {
      ...
      .then((newMeal: Meal) => {
        ...
        setShoppingList([]);
        showToast(
          `Swapped ${day} with: ${newMeal.mealName} (ID: ${newMeal.id})`,
        );
      })
  ...
  const downloadCalendar = () => {
    try {
      ...
      link.href = "/api/mealplan/ics";
      link.download = "mealplan.ics";
      document.body.appendChild(link);
      link.click();
      link.remove();
      window.open(
        "https://calendar.google.com/calendar/u/0/r/import",
        "_blank",
      );
    } catch (err) {
  ...
  return (
    <Box sx={{ p: 3 }} data-testid="meal-plan-component">
      ...
      {isLoadingMealPlan ? (
        ...
        <Typography>
          No meal plan available. Generate a new one to get started.
        </Typography>
      )}
      ...
      <Box sx={{ mt: 2, display: "flex", gap: 2, flexWrap: "wrap" }}>
        ...
        <Button variant="outlined" onClick={finalizePlan} disabled={!mealPlan}>
          Finalize Meal Plan
        </Button>
        <Button
          variant="outlined"
